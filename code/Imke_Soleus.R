...
# load data ####
newNames <- c("WT_1", "WT_3", "WT_5", "WT_9",
"WT_11", "WT_15", "KO_2", "KO_4",
"KO_6", "KO_8", "KO_10", "KO_16")

count <- read_tsv("readcount_genename.xls")%>%
  column_to_rownames(var = "gene_id") %>% 
  dplyr::select (1:12) %>% 
  rename_at(vars(colnames(.)), ~ newNames)
...
ggplot(result_up, aes(x = symbol, y = normalized_counts, fill = genotype)) +
  #geom_dotplot(binaxis='y', stackdir='center',
              # dotsize = 0.2, position=position_dodge(0.8))+
...
# GO analysis with top 20 UP and Down genes upon Nfe2l1 KO
result_GO <- result %>%
  filter(padj < 0.01 &
           baseMean > 100) %>% 
...
dotplot(result_GO,showCategory=15, font.size = 12,title= "GO_Soleus")

result_GO_df <- data.frame(result_GO)

# View mitochondrial genes
mitochondrial_genes <- result_GO_df %>% 
  dplyr::filter(Description == "mitochondrion organization")
mitochondrial_genes$geneID

# proteasome-mediated ubiquitin-dependent protein catabolic process upon Pmepa1 KD ####
myColor <- RColorBrewer::brewer.pal(8, "Dark2")[4]
...
UPS_result <-result_GO_df %>% 
  ...
  #dplyr::filter(Description == "muscle cell differentiation") %>% 
  dplyr::select(geneID) %>% 
...
UPS_normalized_counts <- normalized_counts[UPS_result$ensgene, ] %>% 
  ...
  geom_text(aes(22, 1.00), label = "WT")
...
UPS_normalized_counts <- normalized_counts[UPS_result$ensgene, ] %>% 
  ...
  theme(plot.title = element_text(hjust = 0.5))+
  geom_hline(yintercept = 1, linetype = "dashed")+
  geom_text(aes(28, 1.00), label = "WT")
...
UPS_normalized_counts <- normalized_counts[UPS_result$ensgene, ] %>% 
  ...
  xlab("Genes") +
  ylab("Fold Change") +
  ggtitle("muscle cell differentiation") +
  theme_bw() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  theme(plot.title = element_text(hjust = 0.5))+
  geom_hline(yintercept = 1, linetype = "dashed")+
...
UPS_normalized_counts <- normalized_counts[UPS_result$ensgene, ] %>% 
  ...
  mutate(fold_change = normalized_counts.x / normalized_counts.y) %>% 
  dplyr::select(- ends_with(".y")) %>% 
  filter (fold_change > 1.5 | fold_change < 0.8)    %>% 
  filter(genotype!= "WT") %>% 
  ungroup() %>% 
  mutate(symbol = fct_reorder(symbol, desc(fold_change))) %>% 
  ggplot(aes(x = symbol, y = fold_change, fill = genotype)) +
  ...
  theme(plot.title = element_text(hjust = 0.5))+
  geom_hline(yintercept = 1, linetype = "dashed")+
  geom_text(aes(34, 1.05), label = "WT")
...
UPS_result <-result_GO_df %>% 
  dplyr::filter(Description == "response to oxidative stress") %>% 
  dplyr::select(geneID) %>% 
  separate(geneID,into = paste("gene", 1:220, sep = "/")) %>% 
  gather(key = key, value =  symbol, 1:220)%>%
  right_join(grcm38[,c("symbol", "ensgene")], by = "symbol" ) %>% 
...
UPS_normalized_counts <- normalized_counts[UPS_result$ensgene, ] %>% 
  ...
  mutate(fold_change = normalized_counts.x / normalized_counts.y) %>% 
  dplyr::select(- ends_with(".y")) %>% 
  filter (fold_change > 1 | fold_change < 0.7)    %>% 
  filter(genotype!= "WT") %>% 
  ungroup() %>% 
  mutate(symbol = fct_reorder(symbol, desc(fold_change))) %>% 
  ggplot(aes(x = symbol, y = fold_change, fill = genotype)) +
  ...
  theme(plot.title = element_text(hjust = 0.5))+
  geom_hline(yintercept = 1, linetype = "dashed")+
  geom_text(aes(26, 1.00), label = "WT")
