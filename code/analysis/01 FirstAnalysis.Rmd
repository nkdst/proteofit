...
# rowData
rowdata <- gastroc[, 13:21]
rowdata$gastroc.filtered = rownames(rowdata) %in% rownames(counts.gastroc)
rowdata$soleus.filtered  = rownames(rowdata) %in% rownames(counts.soleus)

se <- SummarizedExperiment(
  assays = list("gastroc" = gastroc[, 1:12], "soleus" = soleus[, 1:12]),
  rowData = rowdata,
...
df <- se[rowData(se)$gastroc.filtered,] %>% assays() %>% .$gastroc
```
...
saveRDS(se,             file = "./data/Robjects/01_se.rds")
