...
Thus a quick look if any of these genes can be simply omitted. Like if the 
gene_type is "other" or "tRNA"

```{r}
# first looking at ENSEMBL:
dup_rows <- duplicated(annot$GeneID) | duplicated(annot$GeneID, fromLast = T)
anot_dup_ENS <- annot[dup_rows, ]
dupENS_perGroup <- dplyr::group_by(anot_dup_ENS, GeneID + gene_biotype) %>% 
  summarise(n = n())

# looking at gene_names(ext):
dup_rows <- duplicated(annot$external_gene_name) | duplicated(annot$external_gene_name, fromLast = T)
anot_dup_geneN <- annot[dup_rows, ]


# are all gene_name duplicates in ENS duplicates?
sum(anot_dup_ENS$GeneID %in% anot_dup_geneN$GeneID) %>%
  sprintf("%d / %d ENSEMBL duplications are also in the gene_name dups", ., nrow(anot_dup_ENS))

diff_ENS_gene <- nrow(anot_dup_geneN) - nrow(anot_dup_ENS)


```

Since all ENSEMBL duplicates are also found in the gene_name duplicates, using
the ENSEMBL as id for the ranks would reduce the number of duplicates by 
`r diff_ENS_gene`.
...
get_fgseaRes <- function(tissue, ranks, maxSize=params$fgsea.maxSize, pathways=CGP, saveResults=T) {
  ...
  if (file.exists(fgseaRes.path) & !params$reevaluate) {
    message("fgsea result found in directory! Prvious version will be loaded ...")
