...
tissue_pal <- c(soleus = "#FFB08E", soleus="#FF6638")
```
...
ranks.soleus <- getRanks(res.gastroc, annot)
...
#   png(barplot.file)
    par(mfrow = c(1, 2))
    barplot(sort(ranks.soleus, decreasing = T), main = "Wald Test ranks, gastroc")
...
fgseaRes.soleus <- get_fgseaRes("gastroc", ranks.gastroc, pathways=CGP)
...
combined_dotplot <-
  function(group,
           ...
           max_label_length = 25, topn=6e6) {
    
    # get pathways of the respective group
    pathways <- filter(fgseaRes.combined, diff.exp == group)$pathway
    
    gastroc.df <- filter(fgseaRes.gastroc, pathway %in% pathways) %>% 
      top_n(topn, wt = abs(NES))
    soleus.df <- filter(fgseaRes.soleus, pathway %in% gastroc.df$pathway)
...
fgsea_dotplot <- function(fgseaRes.df, max_label_length, padj_limits, NES_limits) {
  ...
  fgseaRes_prep.df <- fgseaRes.df %>%
    ...
    mutate(pathway = (
      gsub("_", " ", pathway) %>%
        tolower() %>%
        stringr::str_wrap(width = max_label_length)
    ))
  ggplot(fgseaRes_prep.df,
         aes(x = reorder(pathway, NES), y = NES)) +
    geom_point(aes(size = size, color = padj)) +
    scale_color_gradient(low = "red", high = "blue", limits = padj_limits) +
    scale_size_continuous(range = size_range, breaks = size_breaks, limits=c(10,200)) +
...
for (group in unique(fgseaRes.combined$diff.exp)) {
  ...
  ggsave(paste0("./plots/04_dotplot_", group, ".svg"),
         p_combined)
  print(p_combined)
}
```

#### dotplot: all sensitive
```{r}
for (group in unique(fgseaRes.combined$diff.exp)) {
...
```{r}
plot_top_enrichment(fgseaRes.soleus, CGP, ranks.gastroc, up=T)
...
```{r}
plot_top_enrichment(fgseaRes.soleus, CGP, ranks.gastroc, up=F)
plot_labels <-
...
* [ ] improve fgsea plots\
  ...
  * [ ] better align them ? https://www.biostars.org/p/438985/\
