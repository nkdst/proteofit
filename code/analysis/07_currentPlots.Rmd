...
params:
  ...
  regulated_pal: !r list("upgregulated" = 'royalblue', "downregulated" = 'red', "insignificant" = 'gray')
...
volcanoPlot <- function(result, se, pCutoff = 0.01, FCutoff = 1, tissue = character()) {
  ...
  keyvals <- ifelse(
    result$log2FoldChange < -FCutoff &
      result$padj < pCutoff,
    params$regulated_pal$upgregulated,
    ifelse(
      result$log2FoldChange > FCutoff &
        result$padj < pCutoff,
...
plotCHM <- function(counts_sign) {
  ...
  top_annot <-
    HeatmapAnnotation(
      ...
      col = list(tissue = params$tissue_pal, condition = c("WT" = "white", "KO" = "gray")),
