...
```{r}
# TODO: move to annotation file

annot <- dplyr::rename(annot, GeneID=ensembl_gene_id)

res.gastroc.annot <- as.data.frame(res.gastroc) %>%
    tibble::rownames_to_column("GeneID") %>%
    dplyr::rename(logFC = log2FoldChange, FDR = padj) %>%
    left_join(annot, by=c("GeneID")) %>%
    filter(!is.na(entrezgene_id))

dups.entrez <- res.gastroc.annot$entrezgene_id %>%
  base::duplicated() %>%
  which() %>%
  res.gastroc.annot$entrezgene_id[.]

subset(res.gastroc.annot, entrezgene_id %in% dups.entrez) %>%
  select(GeneID, entrezgene_id, external_gene_name) %>%
...
arrange_plts <- function(plt.list, plt.labels=T) {
  ...
  lims <- list("x" = c(0,17000), "y" = c(-0.8,0.0))
  # for (plt in plt.list) {
  #   # get maximal range for both axes
  #   plt.limx <- layer_scales(plt)$x$range$range
  #   lims$x <- c(max(plt.limx[1], lims$x[1]), min(plt.limx[2], lims$x[2]))
  #   print(plt.limx)
  #   
  #   plt.limy <- layer_scales(plt)$y$range$range
  #   lims$y <- c(min(plt.limy[1], lims$y[1]), max(plt.limy[2], lims$y[2]))
  #   print(plt.limy)
  # }
  
  # for (i in 1:nplts) {
  #   plt.list[i] <- plt.list[i] + coord_cartesian(xlim = lims$x, ylim = lims$y)
  # }
