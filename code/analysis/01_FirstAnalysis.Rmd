...
```{r overview}
plotRowsums <- function(counts.df, tissueName, cutoff = 10) {
  rs.df <- rowSums(counts.df) %>%
    as.data.frame() %>%
    dplyr::rename("rowSums" = ".")
  n.omit <- sum(rs.df < cutoff)
  title <- paste0("rowSums low expressed genes, ", tissueName, "\n",
                  n.omit, " genes will be omitted (~",
                  floor(n.omit / nrow(rs.df) * 100), " %)")
  
  p <- rs.df %>% 
    ggplot(aes(x=log(rowSums + 1)))+
    geom_density(color="black", fill="white")+
    geom_vline(aes(xintercept=log(cutoff + 1), color="red"), linetype="dashed") +
    annotate("text", x=log(10+1), y=0.5, label="rowSums=10", color="red") +
    ggtitle(title) + 
    theme(legend.position = "none", axis.title.y=element_blank())
}

p.soleus <- plotRowsums(counts.soleus, tissueName = "soleus")
p.gastroc <- plotRowsums(counts.gastroc, tissueName = "gastrocnemius")


removeTitle <- function(plt) {
  plt + 
...
ggsave("./plots/01_rowSums_gastroc.svg", removeTitle(p.gastroc), width=16, height=8, units = "cm")
ggsave("./plots/01_rowSums_soleus.svg", removeTitle(p.soleus), width=16, height=8, units = "cm")


# show plots in document
p.gastroc
p.soleus
```
...
```{r fig.width=4, fig.height=4, message=FALSE, eval=FALSE}
# TODO: !! only copied code from 07, needs to be adjusted!
counts.both
...
# TODO: adjust, so that corresponds general color scheme 
plt <-
  autoplot(
    pca,
    data = pca.data,
