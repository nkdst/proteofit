...
Choosing a threshold of `10` can be one method to omit low expressed genes:
...
ggsave("./plots/01_rowsums_combined.svg", p, width=16, height=8, units = "cm")
ggsave("./plots/01_rowsums_combined.png", p, width=16, height=8, units = "cm")
p
```
...
TODO: compare genes of gastroc and soleus. Should only genes be taken, where the threshold was met for both tissues? => Might change things down the pipeline
...
ggsave("./plots/01_filtered_genes.svg", p)
p
```
...
```{r gene_biotypes}
# custom color palette to assign each biotype a unique color (for both treemaps)
...
gene_biotypes_cmp <- merge(gene_biotypes[,c(1,2)], gene_biotypes_filtered[,c(1,2)], by = "biotype", all = TRUE, suffixes = c(".all", ".filt")) %>% 
  mutate("share" = 100 * Freq.filt / Freq.all,
...
plot_single_CHM <- function(counts) {
  # http://colorbrewer2.org/#type=sequential&scheme=BuGn&n=3
  morecols <- colorRampPalette(params$heatmap_pal)
  
  chm <- Heatmap(
    as.matrix(zscore(counts)),
    row_title = "genes",
    name = "zscore",
...
```{r heatmapGastroc, fig.width=4, fig.height=4, message=FALSE}
chm <- plot_single_CHM(counts.gastroc.top)
chm
```
...
```{r heatmapSoleus, fig.width=4, fig.height=4, message=FALSE}
chm <- plot_single_CHM(counts.soleus.top)
chm
```
...
```{r countsBoth, fig.width=4, fig.height=4, message=FALSE, eval=FALSE}
...
```{r combinedHeatmap}
plot_combined_CHM <- function(counts.df, datatype="zscore") {
  # http://colorbrewer2.org/#type=sequential&scheme=BuGn&n=3
  morecols <- colorRampPalette(params$heatmap_pal)
...
```{r heatmapBoth}
chm <- plot_combined_CHM(zscore(counts.both.top), datatype = "zscore")
```


```{r heatmapBothSVG, include=FALSE}
svglite::svglite(
  "plots/01_heatmap_mvg_both.svg",
)
chm
dev.off()
```
...
The union of the top 1000 most variable genes of each tissue is used

```{r pca.both, message=FALSE}
pca <- prcomp(t(counts.both.top), scale. = T)
...
ggsave(filename = "./plots/01_pca_mvg_both.svg", plt)
plt
```
