...
plot.dir <- file.path("plots", "05 GO")
```
...
load("./data/Robjects/03_annot.RData")
```
...
performGO_dotplot <- function(entrez_ids, plot_name, pCutoff = params$pCutoff) {
  ...
  ggsave(
    filename = file.path(
      plot.dir,
      paste0("05_enrichGO_dotplot_", plot_name, ".svg")
    ),
    plot = p
  )
  return(p)
...
To see the effect of not filtering out genes with a baseMean lower than 100:
